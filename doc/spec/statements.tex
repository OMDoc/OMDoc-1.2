...
\svnInfo $Id: statements.tex 8723 2010-09-22 21:10:13Z kohlhase $
...
\begin{tsubsection}[id=symbol-dec]{Symbol Declarations}
  ...
  not exported\twin{export}{symbol} outside the theory; \erratumAdd[reported-by=Florian
  Rabe,date=2006-11-21]{scope is deprecated}{The {\tt{scope}} attribute is deprecated, a
    formalization using the {\tt{hiding}} attribute on the {\tt{imports}} element should
    be used instead}.  Finally, the optional attribute {\attribute{role}{symbol}} that can
...
\begin{tsubsection}[id=axioms]{Axioms}  
  ...
  \erratumReplace[reported-by=Michael Kohlhase,date=2009-10-10]{the {\texttt{for}}
    attribute in the {\texttt{axiom}} element must reference symbol names}{\indextoo{URI}
    references}{names} to symbols declared in the same theory, see {\mylstref{axiom}} for
  an example. Finally, the {\element{axiom}} element can have an {\attribute{type}{axiom}}
...
\begin{tsubsection}[id=definitions]{Definitions}
  ...
  list of names of symbols in the same theory.\erratumAdd[reported-by=Michael
  Kohlhase,date=2009-10-10]{Note that this use of the {\texttt{for}} attribute is
    different from the other usages, which are URI references.} 
...
\item[{\attval{pattern}{type}{definition}}] This is a special degenerate case of the
  ...
  patterns of arguments. Since termination is trivial in this case, no {\element{measure}}
  and {\snippet{ordering}} elements appear in the body.
...
\begin{tsubsection}[id=alternative]{Alternative Definitions}
  ...
  {\attribute{for}{alternative}} on the {\element{alternative }} points to the \erratumReplace
  [date=2007-09-10,reported-by=Kristina Sojakova]{fixed the target of the for attribute}
...
\item[{\attribute{for}{example}}] specifying for which concepts or assertions
    ...
    {\element{definition}}, \erratumAdd[date=2008-07-02,reported-by=Christoph Lange]
    {added the {\tt axiom} element to the list; cf.\ discussion on {\tt omdoc-dev} on May 16, 2008}{\element{axiom}, }\erratumAdd[date=2007-09-10,reported-by=Kristina Sojakova]
    {added the {\tt alternative} element to the list}{\element{alternative}}, or {\element{assertion}} elements.
...
{\attribute{verbalizes}{phrase}}, we can do the following:
...
\begin{tsection}[id=theories]{Theories as Structured Contexts}
  ...
  Theories are specified by the {\eldef{theory}} element in {\omdoc}, which has
  {\erratumReplace[reported-by=Kristina Sojakova,date=2007-07-19]{the {\tt{xml:id}}
      attribute on the {\tt{theory}} element should be optional}{a required}{an optional}}
  ...
  disambiguation on {\element[ns-elt=om]{OMS}} elements (see {\mysecref{openmath}} for a
  discussion).  Additional information about the theory like a title or a short
  description can be given in the {\element{metadata}} element. After this, any
  {\indextoo{top-level}} {\omdoc} element can occur, including the theory-constitutive
  elements introduced in {\mysecsref{statements-constitutive}{definitions}}, even
  {\element{theory}} elements themselves. Note that theory-constitutive elements may
  {\emph{only}} occur in {\element{theory}} elements.

  Theories can be structured like documents e.g. into sections and the like (see
  {\mysecref{sectioning}} for a discussion) via the {\eldef{tgroup}} element, which
  behaves exactly like the {\element{omgroup}} element introduced in
  {\mysecref{sectioning}} except that it also allows theory-constitutive elements, but
  does not allow a {\attribute{theory}{omgroup}} attribute, since this information is
  already given by the dominating {\element{theory}} element.\footnote{This element has
    been introduced to keep {\omdoc} validation manageable: We cannot directly use the
    {\element{omgroup}} element,since there is no simple, context-free way to determine
    whether an {\element{omgroup}} is dominated by a {\element{theory}} element.}
...
\begin{tsubsection}[id=identifying]{OMDoc Theories as Content Dictionaries}
  ...
  \begin{erratum}[reported-by=Michael Kohlhase,date=2008-09-25,type=clarification]{This
    ...
    attribute is the one used for disambiguation on {\element[ns-elt=om]{OMS}} elements
    (see {\mysecref{openmath}} for a discussion).
...
\end{lstlisting}
  ...
  {\attribute[ns-attr=xml]{id}{theory}} attribute has the value {\snippet{elAlg}} and
