...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  exclude-result-prefixes="saxon set exsl om"
  version="1.0">
...
<xsl:variable name="tree">
  <xsl:call-template name="build-signature">
    <xsl:with-param name="externals" select="$cdus[not(@cd=//theory/@xml:id)]"/>
  </xsl:call-template>
</xsl:variable>
    ...
    <xsl:if test="$omduri!=''">
      <thy name="{$cd}" omdoc="{$omduri}">
        ...
        <xsl:for-each select="$syms">
          ...
          <type-for name="{$name}">
            <xsl:copy-of select="$omsyms[@name=$name]/type[@system='sts']/om:OMOBJ/*[1]"/>
          </type-for>
        </xsl:for-each>
      </thy>
    </xsl:if>
